...
export const StatusLookupSection: React.FC<StatusLookupSectionProps> = ({
  ...
  const handleSearch = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      // If 404 / no application found, address is not on whitelist and has not applied
      if (
  ...
  return (
    ...
    >
      <div className="relative w-full bg-[#141922] border border-[#262f3d] rounded-2xl p-6 sm:p-10 shadow-[0_16px_48px_rgba(0,0,0,0.5)]">
        ...
        {errorMessage && (
          <div
            id="status-lookup-error"
            className="p-4 bg-red-950/40 border border-red-500/60 rounded-xl text-red-300 text-xs sm:text-sm flex items-center gap-2.5 mb-6"
          >
        ...
        {/* Case 1: NOT ON WHITELIST AND NOT APPLIED — JOIN EARLY ACCESS */}
        {notApplied && (
          <div
            id="status-not-applied-banner"
            className="p-5 sm:p-6 bg-[#1a1711] border border-[#d97706]/60 rounded-xl space-y-4 animate-fade-in mb-6"
          >
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-lg bg-[#2e210c] border border-[#d97706] flex items-center justify-center shrink-0">
                  <AlertCircle className="w-5 h-5 text-[#f59e0b]" />
                </div>
                <div>
                  <span className="text-[10px] font-arcade text-[#f59e0b] block tracking-wider">
                    VERIFICATION RESULT
                  </span>
                  <h4 className="font-arcade text-base sm:text-lg text-[#fcd34d] tracking-wide">
                    NOT ON WHITELIST
                  </h4>
                </div>
              </div>
              ...
              <button
                ...
                className="w-full sm:w-auto px-5 py-2.5 bg-[#f59e0b] hover:bg-[#fbbf24] text-[#121820] font-arcade font-bold text-xs rounded-lg flex items-center justify-center gap-2 transition-all hover:scale-[1.02] active:scale-[0.98] cursor-pointer shrink-0 shadow-[0_2px_12px_rgba(245,158,11,0.3)]"
              >
            ...
            <p className="text-xs sm:text-sm text-[#d1d5db] font-sans leading-relaxed border-t border-[#3d2a0f] pt-3">
              This wallet address (<span className="font-code text-[#fcd34d]">{searchedQuery}</span>) is not on the whitelist. Please join the early access application below to complete quests and become eligible for the mint!
            </p>
          </div>
        )}

        {/* Case 2: APPLICATION OR WHITELIST FOUND */}
        {result && (
          <div
          ...
          >
            ...
            {result.status === 'Approved' ? (
              /* Case: APPROVED (Whitelisted via CSV or Approved Early Access) */
              <div className="p-4 sm:p-5 bg-green-950/60 border border-green-500/80 rounded-xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 shadow-[0_0_24px_rgba(34,197,94,0.15)]">
                ...
                <div className="px-3 py-1.5 bg-green-900/60 border border-green-400 rounded-md text-[11px] font-arcade text-green-200 shrink-0 flex items-center gap-1.5">
                  ...
                  <span>{result.whitelisted ? 'WHITELIST ELIGIBLE' : 'WHITELIST APPROVED'}</span>
                </div>
              </div>
            ...
            <div className="pt-2 border-t border-[#262f3d] grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs font-code text-[#cbd5e1]">
              <div className="p-3 bg-[#141922] border border-[#262f3d] rounded-lg">
                <span className="text-[10px] text-[#94a3b8] font-arcade block mb-1">
                  {result.whitelisted ? 'SOURCE' : 'APPLICATION ID'}
                </span>
                <span className="font-arcade text-sm text-[#facc15]">
                  {result.whitelisted ? 'WHITELIST (CSV)' : result.application_id}
                </span>
              </div>
              ...
              <div className="p-3 bg-[#141922] border border-[#262f3d] rounded-lg">
                <span className="text-[10px] text-[#94a3b8] font-arcade block mb-1">
                  {result.whitelisted ? 'STATUS' : 'X (TWITTER) HANDLE'}
                </span>
                <span className={result.whitelisted ? 'text-green-300 font-arcade text-xs truncate block' : 'text-[#facc15] truncate block'}>
                  {result.whitelisted ? 'ELIGIBLE FOR MINT' : (result.x_username || '—')}
                </span>
              </div>

              <div className="p-3 bg-[#141922] border border-[#262f3d] rounded-lg">
                <span className="text-[10px] text-[#94a3b8] font-arcade block mb-1">
                  {result.whitelisted ? 'ACCESS TIER' : 'SUBMISSION DATE'}
                </span>
